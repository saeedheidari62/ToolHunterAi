...
def _conservative_confidence(*values):
    ...
    return min(normalized, key=_confidence_rank) if normalized else "NONE"
...
def analyze_price(tool_data, asking_price, market_data=None):
    ...
    if supplied_dynamic and dynamic_valid:
        if not dynamic_values_present:
            dynamic_rejected_reason = "Dynamic market data is valid but incomplete; the static baseline was used."
    ...
    use_dynamic_market = dynamic_valid and dynamic_values_present and dynamic_range_valid and dynamic_confidence_level in ("HIGH", "MEDIUM") and (raw_count is None or dynamic_sample_count >= 2)
    ...
    # A supplied-but-incomplete dynamic payload is explicitly invalid, even if
    # the static knowledge base is otherwise usable. This prevents partial
    # dynamic responses from masquerading as a successful market analysis.
    if supplied_dynamic and dynamic_valid and not dynamic_values_present:
        return {"price_score": 50, "price_status": "UNKNOWN", "price_reason": [dynamic_rejected_reason], "price_difference_percent": None, "market_source": market_source, "market_confidence": market_confidence, "market_benchmark_reason": benchmark_reason}

    low = benchmark.get("used_price_min", benchmark.get("median_price"))
    high = benchmark.get("used_price_max", benchmark.get("median_price"))
    try: asking_price = float(asking_price); low = float(low); high = float(high)
    ...
    if asking_price < low * 0.90: score, status = 80, "VERY_GOOD_PRICE"; reasons.append("The unusually low price should be verified carefully.")
    elif asking_price < low: score, status = 92, "VERY_GOOD_PRICE"; reasons.append("Price is below the normal market range.")
    elif asking_price <= low + (market_reference - low) * 0.25: score, status = 95, "VERY_GOOD_PRICE"; reasons.append("Price is near the low end of the market range.")
    elif asking_price <= market_reference: score, status = 88, "GOOD_PRICE"; reasons.append("Price is below the market average.")
    elif asking_price < high - (high - market_reference) * 0.25: score, status = 78, "FAIR_PRICE"; reasons.append("Price is above the market average but within a reasonable range.")
    elif asking_price <= high: score, status = 65, "HIGH_PRICE"; reasons.append("Price is near the high end of the market range.")
    elif asking_price <= high * 1.10: score, status = 45, "HIGH_PRICE"; reasons.append("Price is above the normal market range.")
    else: score, status = 20, "VERY_HIGH_PRICE"; reasons.append("Price is significantly higher than the market range.")
