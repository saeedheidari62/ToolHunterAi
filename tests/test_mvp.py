...
def test_bosch_gbh_2_26():
    run_case("Bosch GBH 2-26", {"title": "Bosch GBH 2-26", "description": "دریل بتن کن بوش GBH 2-26 سالم با امکان تست", "price": 8500000, "seller_type": "personal", "testing": True, "warranty": False, "condition": "used"}, "bosch_gbh_2_26")


def test_bosch_gsh500():
    run_case("Bosch GSH500", {"title": "Bosch GSH500", "description": "بتن کن بوش مدل GSH500 سالم", "price": 9000000, "seller_type": "personal", "testing": False, "warranty": False, "condition": "used"}, "bosch_gsh500")


def test_makita_hr2470():
    run_case("Makita HR2470", {"title": "Makita HR2470", "description": "بتن کن ماکیتا 2470 سالم", "price": 8000000, "seller_type": "personal", "testing": True, "warranty": False, "condition": "used"}, "makita_hr2470")
...
def test_decision_boundaries(monkeypatch):
    ...
    monkeypatch.setattr(decision_engine, "load_tool", lambda _: {"risk": {"score": 0}, "buy_score": 100, "market": {"used_price_min": 100, "used_price_max": 100}})
    ...
    monkeypatch.setattr(decision_engine, "analyze_price", lambda *_args, **_kwargs: {"price_status": "GOOD_PRICE", "price_score": 50, "price_difference_percent": 0, "price_reason": []})
    base = {"tool_name": "bosch_gbh_2_26", "asking_price": 100, "has_test": True, "has_warranty": False, "description": "", "ad_score": 50, "analysis": []}
    result = decision_engine.make_decision(base)
    assert result["decision"] == "BUY", result
    review_data = dict(base)
    review_data["ad_score"] = 50
    monkeypatch.setattr(decision_engine, "load_tool", lambda _: {"risk": {"score": 20}, "buy_score": 75, "market": {"used_price_min": 100, "used_price_max": 100}})
    ...
    monkeypatch.setattr(decision_engine, "load_tool", lambda _: {"risk": {"score": 80}, "buy_score": 50, "market": {"used_price_min": 100, "used_price_max": 100}})
...
def test_end_to_end_dre_dynamic_market(monkeypatch):
    ...
    monkeypatch.setattr(api, "get_dynamic_market_data", lambda *_args, **_kwargs: {"valid": True, "sample_count": 2, "min_price": 12000000.0, "max_price": 14900000.0, "median_price": 13450000.0, "confidence": "MEDIUM", "variant": "DRE"})
    result = analyze_single_ad({"title": "دریل بتن‌کن بوش آلمانی GBH 2-26 DRE Professional", "description": "دریل بتن کن بوش GBH 2-26 DRE در حد نو", "price": 14900000, "seller_type": "shop", "testing": True, "warranty": False, "condition": "used"})
...
def test_end_to_end_dfr_low_confidence_fallback(monkeypatch):
    from backend import api
    monkeypatch.setattr(api, "get_dynamic_market_data", lambda *_args, **_kwargs: {"valid": True, "sample_count": 1, "min_price": 27500000.0, "max_price": 27500000.0, "median_price": 27500000.0, "confidence": "LOW", "variant": "DFR"})
    result = analyze_single_ad({"title": "بتن کن بوش GBH 2-26 DFR", "description": "بتن کن بوش GBH 2-26 DFR سالم", "price": 27500000, "seller_type": "personal", "testing": False, "warranty": False, "condition": "used"})
    assert result["tool"] == "bosch_gbh_2_26", result
...
def test_unknown_tool_returns_structured_error():
    result = analyze_single_ad({"title": "Makita SuperTool XYZ 9999", "description": "ابزار ناشناخته تست", "price": 1000000, "seller_type": "personal", "condition": "used"})
...
def test_ai_tool_resolver_fallback(monkeypatch):
    ...
    monkeypatch.setattr(api.ai_tool_resolver, "resolve", lambda text: {"tool_id": "bosch_gbh_2_26", "confidence": 0.96, "evidence": ["GBH 2-26 model identified from marketplace text"]})
    monkeypatch.setattr(api, "get_dynamic_market_data", lambda *_args, **_kwargs: None)
    result = analyze_single_ad({"title": "دریل بتن کن بوش مدل بیست و شش", "description": "سالم با امکان تست", "price": 8500000, "seller_type": "personal", "testing": True, "warranty": False, "condition": "used"})
    assert result["tool"] == "bosch_gbh_2_26", result
